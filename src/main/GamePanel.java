...
public class GamePanel extends JPanel implements KeyListener
{
    ...
    private Platform entPlatform;              //Brick entPlatform declares an instance of the object Brick
    ...
    private PowerUp nrmWidthPowerUp;
    private Random genRndPowerUp;
    ...
    {
        nrmWidthPowerUp = new PowerUp();
        entBricks = new ArrayList<>();
        if (!boolIsSetHardmode)
        {
            entPlatform = new Platform(230, 480, 150, 25);
            nrmWidthPowerUp.setFallSpeed(2);
        } else if (boolIsSetHardmode)
        {
            entPlatform = new Platform(250, 480, 100, 25);
            nrmWidthPowerUp.setFallSpeed(3);
        }
    ...
    public void paintComponent(Graphics graphics)
    {
        ...
        nrmWidthPowerUp.drawPowerUp(graphics, this);
    }
    ...
    public void updateGameState()
    {
        ...
        for (int i = 0; i < entBricks.size(); i++)
        {
            ...
            if (entBall.intersects(entBrick) && !entBrick.isDestroyed())
            {
                ...
                if(genRndPowerUp.nextInt(12)+1 > 11)
                {
                    if(!nrmWidthPowerUp.isSpawnedPUP())
                    {
                        nrmWidthPowerUp.spawnPowerUp(entBrick.getXPos(), entBrick.getYPos(), 100, 100, "Resources\\Images\\powerup.png");
                    }
                }
        ...
        if (entPlatform.intersects(nrmWidthPowerUp))
        {
            entPlatform.widthPowerUp(nrmWidthPowerUp.getPowerUp());
            nrmWidthPowerUp.setDestroyed(true);
        }

        nrmWidthPowerUp.y += nrmWidthPowerUp.getFallSpeed();

        if (nrmWidthPowerUp.isSetDestroyed())
        {
            nrmWidthPowerUp.reducePUPDuration();
        }
        if (nrmWidthPowerUp.isEmptyPUP())
        {
            entPlatform.resetWidth();
        }
